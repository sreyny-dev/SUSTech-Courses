...
def handlePost(client_socket, request_data, auth_header, session_id, raw_data):
    ...
    try:
        if url == "/upload":
            if os.path.exists(os.path.join(path, os.pardir)):
                ...
                if start_index != -1:
                    # Trim request_data to end at the \r\n\r\n sequence
                    new_head = raw_data[:start_index]
                    raw_data = raw_data[start_index + len(b'\r\n\r\n'):]
                end_index = raw_data.find(b'\r\n--' + content_boundary.encode('utf-8') + b'--\r\n')
                if end_index != -1:
                    raw_data = raw_data[:end_index]
                new_header = new_head.decode('utf-8').split('\n')
                file_line = [h for h in new_header if h.startswith('Content-Disposition:')]
                ...
                # print(content)
                # content = content[1:]
                with open(os.path.join(path, content_title), 'wb') as file:
                        file.write(raw_data)
        ...
        while True:
            ...
            while True:
                try:
                    ...
                    raw_request_data += chunk  # Append the received chunk to the request data
                    if b'\r\n\r\n' in chunk:
            ...
            if not raw_request_data:
                continue
            end_index = raw_request_data.find(b'\r\n\r\n')
            if end_index != -1:
                # Trim request_data to end at the \r\n\r\n sequence
                request_data = raw_request_data[:end_index]
                raw_data = raw_request_data[end_index + len(b'\r\n\r\n'):]
            if not request_data:
                continue
            ...
            if cookie:
                ...
                try:
                    ...
                    if session_id in session:
                        print("Check Time...")
                        s_id, expired = session[session_id]
            ...
            if method == 'GET':
                ...
                try:
                    type_line = [h for h in headers if h.startswith('Content-Type:')]
                    _, content_boundary = type_line[0].split("; boundary=")
                    while content_boundary.encode('utf-8') + b'--\r\n' not in raw_data:
                        try:
                            chunk = client_socket.recv(4096)
                            raw_data += chunk  # Append the received chunk to the request data
                            if content_boundary.encode('utf-8') + b'--\r\n' in chunk:
                                break  # Break the loop if no more data is received
                        except ConnectionResetError:
                            print("Connection was reset by the peer.")
                            break
                    ...
                    pass
                handlePost(client_socket, request_data, auth[0], session_id, raw_data)
